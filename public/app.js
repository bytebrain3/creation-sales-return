document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", () => {
    if (document.activeElement !== manualInvoiceIdInput && document.activeElement !== reasonInput) {
      barcodeInput.focus()
    }
  })
  ...
  function clearAllInvoices() {
    ...
    if (confirm(`Are you sure you want to clear all ${invoices.length} invoices?`)) {
      invoices.length = 0
      updateInvoiceList()
  ...
  async function processInvoices() {
    ...
    try {
      ...
      try {
        ...
        alert("Could not connect to server. Using mock data for display purposes.")
      }
      ...
      showSuccess(`Processing complete! Successfully processed ${successCount} invoices with ${failedCount} failures.`)
    } catch (err) {
  ...
  function testErrorHandling() {
    ...
    fetch("/api/test-errors", {
      ...
      .then((data) => {
        ...
        lastProcessedResults = data
        displayResults(data)
        showSuccess("Error handling test complete. All invoices should show as failed with test errors.")
      })
      .catch((err) => {
        ...
        displayResults(mockData)
        showSuccess("Using mock data for error testing. All invoices should show as failed.")
      })
      .finally(() => {
        loadingOverlay.style.display = "none"
      })
  ...
  function downloadResults() {
    ...
    successful.forEach((invoice) => {
      csvContent += `${invoice.id},${(invoice.reason || "").replace(/,/g, ";")},${invoice.customerName || "N/A"},${invoice.timestamp || ""}\r\n`
    })
    ...
    failed.forEach((invoice) => {
      csvContent += `${invoice.id},${(invoice.reason || "").replace(/,/g, ";")},${(invoice.error || "").replace(/,/g, ";")}\r\n`
    })

    const encodedUri = encodeURI(csvContent)
    const link = document.createElement("a")
    link.setAttribute("href", encodedUri)
    link.setAttribute("download", `invoice-results-${new Date().toISOString().slice(0, 10)}.csv`)
