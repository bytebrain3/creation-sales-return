...
function buildChromeLaunchOptions(executablePath) {
  const profileDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "creation-sales-return-chrome-"),
  );
  ...
  return {
    ...
    args: [
      ...
      "--disable-features=PasswordManagerOnboarding,PasswordLeakDetection,PasswordChangeDetection,PasswordManagerRedesign,PasswordManagerAccountStorage",
      "--disable-save-password-bubble",
    ],
  };
}

async function dismissBrowserPrompts(page) {
  try {
...
async function handleSweetAlertIfPresent(page) {
  ...
  const swalConfirm = await page.$(
    ".swal2-confirm, .swal2-actions button.swal2-confirm, button.confirm",
  );
  if (swalConfirm) {
    try {
...
async function robustClick(page, selector) {
    ...
    .waitForFunction(
      (sel) => {
        ...
        return (
          ...
          (el.contains(topEl) ||
            topEl === el ||
            el.contains(topEl.closest?.("button") || topEl))
        );
      },
      { timeout: 7000 },
      selector,
  ...
  try {
    ...
    throw new Error(`All clicks failed ${selector}: ${e.message}`);
  }
}
...
async function processInvoice(page, id, reason, attempt = 1) {
  ...
  try {
    console.log(
      `🔄 [${id}] Processing invoice${attempt > 1 ? ` (retry ${attempt})` : ""}`,
    );
    ...
    await page.evaluate(() => {
      const input = document.querySelector(
        "#sale-table_filter input.form-control-sm",
      );
      if (input) {
        input.focus();
        input.value = "";
    ...
    await page
      .waitForFunction(
        (invoiceId) => {
          ...
          const txt =
            document.querySelector("#sale-table tbody")?.innerText || "";
          return (
            txt.includes(invoiceId) ||
            info.textContent.includes("Showing 0 to 0")
          );
        },
    ...
    const hasDropdown = await page.evaluate(
      () => !!document.querySelector("#sale-table tbody tr .dropdown-toggle"),
    );
    if (!hasDropdown)
      return {
        success: false,
    ...
    await page
      .waitForFunction(
        () =>
          !!document.querySelector('textarea[name="return_note"]') ||
    ...
    await page
      .click('textarea[name="return_note"]', { clickCount: 3 })
      .catch(() => {});
    await page.evaluate(() => {
    ...
    try {
      await robustClick(page, submitSelector);
    } catch (e) {
      return {
        success: false,
        error: `Failed to click submit button: ${e.message}`,
      };
    }
    ...
    const urlChange = page
      .waitForFunction(() => !location.href.includes("/return-sale/invoice"), {
        timeout: 25000,
      })
      .catch(() => null);
    ...
    console.log(
      `✅ [${id}] Submitted, URL: ${curUrl} (${((Date.now() - t0) / 1000).toFixed(1)}s)`,
    );
    if (curUrl.includes("/return-sale/storecustom"))
      return {
        success: false,
        error:
    ...
    const toastErr = await page
      .evaluate(() => {
        const el = document.querySelector(
          ".alert-danger, .toast-error, .swal2-html-container, .alert.alert-danger",
        );
        return el ? el.innerText?.slice(0, 250).trim() : null;
      })
      .catch(() => null);
    if (toastErr && /error|failed|invalid|something went wrong/i.test(toastErr))
      return { success: false, error: toastErr };

    // Success: look for success toast
    const toastOk = await page
      .evaluate(() => {
        const el = document.querySelector(
          ".alert-success, .toast-success, .swal2-html-container",
        );
        if (el && /success|returned|completed/i.test(el.innerText || ""))
    ...
    const retriable =
      /not clickable|not an Element|Timeout|navigation|detached/i.test(
        err.message,
      );
    ...
    try {
      await page.screenshot({
        path: path.join(__dirname, `error-${id}-${Date.now()}.png`),
        fullPage: false,
      });
    } catch {}
  ...
  try {
    ...
    if (!exe) {
      ...
      for (const { id, reason } of invoices)
        fail.push({ id, reason, error: msg });
      return { successful: ok, failed: fail };
    }
    console.log(`Launching browser: ${exe}`);
    browser = await puppeteer.launch(buildChromeLaunchOptions(exe));
    ...
    try {
      ...
      await dismissBrowserPrompts(page);
      await Promise.all([
        page
          .waitForNavigation({
            waitUntil: "domcontentloaded",
        ...
        .waitForFunction(
          () =>
            ...
            !!document.querySelector(
              ".main-sidebar, #sale-table, [href*='/logout']",
            ),
          { timeout: 10000 },
        )
        .catch(() => {});
      ...
      for (const { id, reason } of invoices)
        fail.push({ id, reason, error: `Login failed: ${e.message}` });
      return { successful: ok, failed: fail };
    }
    ...
    page.on("dialog", async (d) => {
      try {
        ...
        if (
          d.type() === "confirm" &&
          msg.toLowerCase().includes("are you sure")
        ) {
          await d.accept();
    ...
    for (const { id, reason } of invoices) {
      ...
      console.log(
        `⏳ Progress: ${ok.length + fail.length}/${invoices.length} (✓${ok.length} ✗${fail.length})`,
      );
    }
    console.log(
    ...
    console.log(
      `Total: ${invoices.length}  Successful: ${ok.length}  Failed: ${fail.length}`,
    );
    ...
    for (const { id, reason } of invoices.filter((x) => !doneIds.has(x.id)))
      fail.push({ id, reason, error: `Fatal: ${err.message}` });
    return { successful: ok, failed: fail };
  } finally {
    if (browser)
      try {
...
app.post("/api/process-invoices", async (req, res) => {
  try {
    ...
    res.status(500).json({
      ...
      failed: (req.body?.invoices || []).map((inv) => ({
        ...inv,
        error: e.message,
      })),
    });
